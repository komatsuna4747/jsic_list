...
class JSICDefinitionExtractor:
    ...
    def get_jsic_code(self) -> None:
        ...
        # Major groups
        code_major_group = [
            major_group.text.split()[1] for major_group in soup.find_all("h3")
        ]
    ...
    def get_jsic_definition(self, classification: str) -> pd.DataFrame:
        ...
        if classification not in ["division", "major_group", "group", "detail"]:
            raise ValueError(
                "'classification' must be either 'division', 'major_group', 'group', or 'detail'."
            )
        ...
        for count, code in enumerate(code_jsic):
            if (count + 1) % 10 == 0:
                logger.info(
                    f"Progress of extracting JSIC {classification}: {count + 1} / {len(code_jsic)}"
                )
            ...
            for key, value in dict_raw.items():
                if "説明" in key:
                    dict_jsic[f"description_{classification}"] = (
                        value.replace("　", "").replace(" ", "").replace("総説", "")
                    )
